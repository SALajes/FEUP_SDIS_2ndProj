...
public class ReclaimProtocol {
    ...
    public static void receiveRemoved(RemovedMessage removedMessage) {
        ...
        if (Store.getInstance().checkBackupChunksOccurrences(chunk_id) != -1) {
            ...
            Store.getInstance().removeStoredChunkOccurrence(chunk_id, removedMessage.getSender());
            ...
            if (!Store.getInstance().hasReplicationDegree(chunk_id)) {
                ...
                if (chunk != null) {
                    Runnable task = () -> sendPutchunk(removedMessage.getSender(), Store.getInstance().getReplicationDegree(chunk_id), file_id, chunk);
